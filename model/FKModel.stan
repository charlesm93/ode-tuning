...
data {
  ...
  int<lower = 0, upper = 1> stiff_solver;
}
...
transformed data {
  ...
  // Fix gamma parameter for now
  real<lower = 0> gamma = 0.17;

  // tuning parameters for ODE solvers
  // CHECK -- adjust for stiff vs nonstiff case?
  real rel_tol = 1e-6;
  real abs_tol = 1e-6;
  ...
  //   rel_tol = 1e-10;
  //   abs_tol = 1e-10;
  //   max_num_steps = 100000000;  // 1e8
  // } else {
  //   rel_tol = 1e-6;
  //   abs_tol = 1e-6;
  //   max_num_steps = 1000000;  // 1e6
  // }
}
...
transformed parameters {
  ...
  // matrix to store drug mass and neutrophil count.
  {
    ...
    if (!stiff_solver) {
      ...
      x = pmx_solve_twocpt_bdf(reduced_system, nOde,
                               time, amt, rate, ii, evid, cmt, addl, ss,
                               theta, biovar, tlag,
                               rel_tol, abs_tol, max_num_steps);
    }
